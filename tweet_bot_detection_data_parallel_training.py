...
USER_NAME = {'local':LOCAL_USERNAME,'server':HADOOP_USERNAME} # {'local':'hadoop','server':'hadoop'}
...
# # for spark-submit
spark = SparkSession.builder.appName('ml_account_ base_session2').getOrCreate()
# spark
...
def read_dataset():
    bot_tweets = spark.read.csv(bot_tweets_dataset_path, header = True, inferSchema = True).limit(10000)
    genuine_tweets = spark.read.csv(genuine_tweets_dataset_path, header = True, inferSchema = True).limit(10000)
...
def training_model(X, Y, worker_id):
    for i in range(EPOCHS):
        ##load data weight and gradient
        ...
        print("({} --> {}):: epoch completed".format(worker_id, i))
...
def get2DenseLayeredModel(input_dim):
    ...
    model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy']) #Adam()
...
def loadFromHDFS(foldername, worker_id):
    ...
    #find aggregated average of gradients
    if foldername['server'] == 'gradients':
        sz = len(grad_list)
        for idx in range(len(grad_sum)):
            tensor_div = tf.constant(sz, shape=grad_sum[idx].shape, dtype=grad_sum[idx].dtype)
            grad_sum[idx] = tf.math.divide_no_nan(grad_sum[idx], tensor_div)
...
#     print(len(tweets_updated_df.collect()), tweets_updated_df.columns)
    ...
    scaled_df = doDataScaling(tweets_updated_df, "independent_features", "scaled_independent_features").drop("independent_features")

    norm_df = doDataNormalization(scaled_df, "scaled_independent_features", "norm_independent_features").drop("scaled_independent_features")

    
#     tweets_updated_df = tweets_updated_df.cache()
    
    X_test, Y_test = to_nparray_dataset(norm_df, "norm_independent_features", 'BotOrNot')
...
def worker_task_with_epoch(bot_tweets_df, genuine_tweets_df, worker_id):
...
#     tweets_updated_df = tweets_updated_df.cache()
    ...
    X, Y = to_nparray_dataset(norm_df, "norm_independent_features", 'BotOrNot')

##    X, Y = SMOTEENN(X, Y)
    
    training_model(X, Y, str(worker_id))
...
def ApplicationJob():
    ...
    for idx in range(num_of_model):
        thread = InheritableThread(target = worker_task_with_epoch, kwargs={'bot_tweets_df': bot_dfs[idx],
